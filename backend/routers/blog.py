...
def get_material(material_id: int, db: Session = Depends(get_db)):
    material = blog_service.get_material(db, material_id)
    if material is None:
...
def get_generation(generation_id: int, db: Session = Depends(get_db)):
    generation = blog_service.get_generation(db, generation_id)
...
def delete_generation(generation_id: int, db: Session = Depends(get_db)):
    ok = blog_service.delete_generation(db, generation_id)
    if not ok:
        raise HTTPException(status_code=404, detail="生成记录不存在")
