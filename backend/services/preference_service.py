...
def _log_path(user_id: int, date_str: str | None = None) -> str:
    if date_str is None:
        date_str = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d")
    return str(LOG_DIR / f"{user_id}_{date_str}.txt")


def _today_log_path(user_id: int) -> str:
    return _log_path(user_id)
...
def analyze_user_preferences_for_date(db: Session, user_id: int, date_str: str) -> dict:
    """与 analyze_user_preferences 相同，但可指定日期."""
    debug: dict = {"steps": []}

    log_path = _log_path(user_id, date_str)
    plan_ids, liked_lines = _parse_liked_plan_ids(log_path)
    debug["steps"].append({
        "step": 1, "action": "read_log",
        "log_path": log_path,
    ...
    if not plan_ids:
        debug["message"] = f"no likes on {date_str}"
        debug["prototypes"] = 0
        return debug
