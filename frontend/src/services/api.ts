...
export const api = {
  ...
  actOnAdvice: (id: number, action: 'accept' | 'reject' | 'revise', additional_requirement = '', selected_itinerary_ids: number[] = []) => request<Advice>(`/ai-advice/${id}/action`, { method: 'POST', body: JSON.stringify({ action, user_id: requireUserId(), additional_requirement, selected_itinerary_ids }) }),
