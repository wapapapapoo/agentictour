...
async function generate(importToCompanion = false) { error.value = ''; importMsg.value = ''; naturalLanguage.value = ''; if (!form.value.start_date || !form.value.end_date) { error.value = '请先选择出行日期。'; return }; if (form.value.end_date < form.value.start_date) { error.value = '返程日期不能早于出发日期。'; return }; loading.value = true; try { plan.value = await api.generatePlan({ ...form.value, trip_id: 0 }); if (importToCompanion) { try { const result = await api.syncPlanItineraries(plan.value.id); importMsg.value = `规划已生成，并导入 ${result.created_count} 项可执行日程。`; await loadHistory(); await router.push('/companion'); return } catch (cause) { error.value = `规划已经生成，但导入旅行陪伴失败：${cause instanceof Error ? cause.message : '请稍后重试。'}` } } await loadHistory() } catch (cause) { error.value = cause instanceof Error ? cause.message : '行程生成失败，请稍后重试。' } finally { loading.value = false } }
...
<template>
  <div class="page planner-page">
    <PlanPreviewModal v-if="historyPreview" :plan="historyPreview" :revising="historyRevisionLoading" :error="historyRevisionError" @close="historyPreview = null" @revise="reviseHistory" @edit="editHistoricalPlan" @import="importPlan(historyPreview, true)" />
    <section class="hero">
      <div>
        <p class="eyebrow">
    ...
    <div class="planner-grid">
      <section class="card form-card">
        ...
        >
          ...
          @click="generate(true)"
        >
          {{ loading ? '正在检查并生成…' : '生成并导入旅行陪伴 →' }}
        </button></div><small class="generation-hint">两种方式都会先检查日期冲突；只有选择导入，生成的日程才会进入旅行陪伴。</small><p
          v-if="error"
          class="error"
        >
          {{ error }}
    ...
    <p v-if="importMsg" class="import-message">{{ importMsg }}</p>
    <section
      v-if="editingTrip"
