...
async function generate() { if (loading.value) return; error.value = ''; importMsg.value = ''; naturalLanguage.value = ''; if (!form.value.start_date || !form.value.end_date) { error.value = '请先选择出行日期。'; return }; if (form.value.end_date < form.value.start_date) { error.value = '返程日期不能早于出发日期。'; return }; loading.value = true; try { plan.value = await api.generatePlan({ ...form.value, trip_id: 0 }); try { const result = await api.syncPlanItineraries(plan.value.id); plan.value.companion_imported = true; importMsg.value = `规划已生成，并导入 ${result.created_count} 项可执行日程。`; await loadHistory(); await router.push('/companion'); return } catch (cause) { error.value = `规划已经生成，但导入旅行陪伴失败：${cause instanceof Error ? cause.message : '请稍后重试。'}` } await loadHistory() } catch (cause) { error.value = cause instanceof Error ? cause.message : '行程生成失败，请稍后重试。' } finally { loading.value = false } }
