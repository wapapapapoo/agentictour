    ...
    >
      <div>
        ...
        >
          ...
          class="secondary import-plan"
          :disabled="importingPlanId === plan.id"
          @click="importPlan(plan, true)"
        >
          {{ importingPlanId === plan.id ? '正在导入…' : '导入并进入旅行陪伴' }}
        </button><button
          class="danger"
          @click="removePlan"
        >
...
.import-plan{border-color:#8cbfa2;background:#edf8f1}.import-message{margin:12px 0 0;border-radius:10px;padding:10px 13px;background:#eef8f1;color:#39725a;font-size:13px}
</style>
