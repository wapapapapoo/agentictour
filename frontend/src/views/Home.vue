...
async function generate() { error.value = ''; importMsg.value = ''; naturalLanguage.value = ''; if (!form.value.start_date || !form.value.end_date) { error.value = '请先选择出行日期。'; return }; if (form.value.end_date < form.value.start_date) { error.value = '返程日期不能早于出发日期。'; return }; loading.value = true; try { plan.value = await api.generatePlan({ ...form.value, trip_id: 0 }); try { const result = await api.syncPlanItineraries(plan.value.id); importMsg.value = `规划已生成，并导入 ${result.created_count} 项可执行日程。`; await loadHistory(); await router.push('/companion'); return } catch (cause) { error.value = `规划已经生成，但导入旅行陪伴失败：${cause instanceof Error ? cause.message : '请稍后重试。'}` } await loadHistory() } catch (cause) { error.value = cause instanceof Error ? cause.message : '行程生成失败，请稍后重试。' } finally { loading.value = false } }
...
async function openHistory(item: Plan) { loading.value = true; error.value = ''; try { historyPreview.value = { ...(await api.getPlan(item.id)), companion_imported: item.companion_imported } } catch (cause) { error.value = cause instanceof Error ? cause.message : '读取失败。' } finally { loading.value = false } }
...
async function openTripEditor(target: Plan | null = plan.value) { if (!target) return; try { const trip = await api.getTrip(target.trip_id); editingPlanId.value = target.id; tripDraft.value = { id: trip.id, title: trip.title, origin_city: trip.origin_city, destination_city: trip.destination_city, start_date: trip.start_date, end_date: trip.end_date, status: trip.status === 'cancelled' ? 'cancelled' : 'planned' }; editingTrip.value = true; await nextTick(); globalThis.document.querySelector('.trip-editor')?.scrollIntoView({ behavior: 'smooth', block: 'center' }) } catch (cause) { error.value = cause instanceof Error ? cause.message : '无法读取行程详情。' } }
async function editHistoricalPlan() { const target = historyPreview.value; historyPreview.value = null; await openTripEditor(target) }
async function saveTrip() { try { const updated = await api.updatePlan(editingPlanId.value, { title: tripDraft.value.title, origin_city: tripDraft.value.origin_city, destination_city: tripDraft.value.destination_city, start_date: tripDraft.value.start_date, end_date: tripDraft.value.end_date, status: tripDraft.value.status }); if (plan.value?.id === updated.id) plan.value = updated; if (historyPreview.value?.id === updated.id) historyPreview.value = updated; editingTrip.value = false; await loadHistory() } catch (cause) { error.value = cause instanceof Error ? cause.message : '保存行程失败。' } }
async function importPlan(target: Plan | null, enterCompanion = false) { if (!target || importingPlanId.value) return; importingPlanId.value = target.id; importMsg.value = ''; try { const result = await api.syncPlanItineraries(target.id); importMsg.value = result.created_count ? `已导入 ${result.created_count} 项日程。` : '这份规划已经导入，没有产生重复日程。'; if (enterCompanion) await router.push('/companion') } catch (cause) { importMsg.value = cause instanceof Error ? cause.message : '导入失败。' } finally { importingPlanId.value = 0 } }
  ...
  <div class="page planner-page">
    ...
    <div class="planner-grid">
      <section class="card form-card">
        ...
        /></label><div class="generation-actions">
          <button
            class="primary"
            :disabled="loading"
            @click="generate"
          >
            {{ loading ? '正在生成并导入…' : '生成旅行规划' }}
          </button>
        </div><small class="generation-hint">生成后会自动导入旅行陪伴，形成可继续编辑的实时日程。</small><p
          v-if="error"
          class="error"
        >
          {{ error }}
