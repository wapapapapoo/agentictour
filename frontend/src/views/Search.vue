...
const topLiked = ref<Array<{ plan_id: number; destination: string; like_count: number }>>([])
...
async function loadTrends() {
  try {
    ...
    topLiked.value = data.top_liked_posts.slice(0, 5)
  } catch {
    trends.value = []
    topLiked.value = []
  }
...
<template>
  <div class="search-page">
    <section class="search-hero">
      ...
      <h1>从别人的旅程里，<br class="title-break">找到你的下一站。</h1>
      ...
      <div v-if="topLiked.length" class="suggestions">
        <span>热门帖子</span>
        <span v-for="item in topLiked" :key="item.plan_id" class="trending-chip">🔥 {{ item.destination }} <small>{{ item.like_count }} 赞</small></span>
      </div>
    </section>
