    ...
    <div class="main-grid">
      ...
      <aside class="side-stack">
        <section class="card advice-card">
          ...
          <article v-for="item in replanAdvice.slice(0, 5)" :key="item.advice_id" class="advice-item">
            ...
            <div class="markdown-body compact" v-html="renderMarkdown(stripThink(item.advice_text))"></div><small v-if="item.audit_reason" class="audit-reason">{{ item.audit_reason }}</small>
          ...
          <p v-if="!replanAdvice.length" class="muted">输入变化原因后获取待确认的日程调整方案。</p>
        </section>
